...
def problem2_3(ne):
    ...
    for x in range(0, num_states):
        ...
        num_char = len(state_name)
        for y in range(0, num_char):
            y = y + 1
        print("has",y,"letters.")
...
def problem2_4():
    ...
    for x in range(0,10):
        ...
        rand_mul = rand_num * 5
        rand_sum = rand_mul + 30
        rand_list.append(rand_sum)
    print(rand_list)
...
def problem2_8(temp_list):
    sum_ = 0
    num_list = len(temp_list)
    for x in temp_list:
        sum_ = sum_ + x
    average = sum_/num_list
